...
```{r}
tab <- geobr::read_state(showProgress = F, year = 2020) %>% as_tibble() %>% 
  mutate(code_state = as.character(code_state)) %>% 
  select(code_state, abbrev_state, name_region)
```
...
# deflacionando
df_combustivel$preco_def <- deflateBR::ipca(df_combustivel$preco, as.Date(df_combustivel$date), "07/2024")

df_combustivel <- df_combustivel %>% 
  left_join(tab, by = join_by("UF" == "code_state"))
```
...
```{r}
sumario_preco <- df_gasolina %>% 
  filter(!is.na(preco_def), 
         UF %in% c(29, 13), 
         as.Date(date) < "2021-12-01") %>% 
  summarise(n = n(),
            Mean = mean(preco_def),
            `Std. Dev` = sd(preco_def), 
            Min = min(preco_def), 
            Max = max(preco_def),
            .by = c(abbrev_state)) %>% 
  mutate(date = "< 12/2021") %>% 
  rbind(df_gasolina %>% 
      filter(!is.na(preco_def), 
             UF %in% c(29, 13), 
             (as.Date(date) >= "2021-12-01" & as.Date(date) < "2022-12-01")) %>% 
      summarise(n = n(),
                Mean = mean(preco_def),
                `Std. Dev` = sd(preco_def), 
                Min = min(preco_def), 
                Max = max(preco_def),
                .by = c(abbrev_state)) %>% 
      mutate(date = "12/2021 - 11/2022")) %>% 
  rbind(df_gasolina %>% 
      filter(!is.na(preco_def), 
             UF %in% c(29, 13), 
             (as.Date(date) >= "2022-12-01" & as.Date(date) < "2023-12-01")) %>% 
      summarise(n = n(),
                Mean = mean(preco_def),
                `Std. Dev` = sd(preco_def), 
                Min = min(preco_def), 
                Max = max(preco_def),
                .by = c(abbrev_state)) %>% 
      mutate(date = "12/2022 - 11/2023")) %>% 
  arrange(date) %>% 
  mutate(Produto = "Gasolina A")
```


```{r}
df_gasolina %>% 
  filter(Cidade %in% c("Betim", "São Francisco Do Conde"), !is.na(preco)) %>% 
  mutate(Cidade = paste(Cidade, abbrev_state, sep = " - ")) %>% 
  ggplot(aes(x = data, y = preco, group = factor(Cidade), color = factor(Cidade)))+
  geom_point(group = "refinaria")+
  geom_line()+
  geom_vline(xintercept = "2021-12", linetype = 2, colour = "black")+
  labs(color = "Cidade")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))


ggsave("comparacao_preços_gas.png", 
       path = "S:/CECAN/Felipe Raposo/Refino/Sobrepreço/[refino] imagens")
```

Preço médio dos grupos de tratamento-controle, levando em conta a refinaria

```{r}
# quais os coortes da gasolina
coorte_gas <- df_gasolina %>% 
  filter(!is.na(preco), 
         priv == 1) %>% 
  group_by(code_cidade) %>% 
  summarise(data_coorte = min(date)) %>% 
  ungroup() %>% 
  reframe(.by = data_coorte) %>% 
  mutate(data_coorte = as.Date(data_coorte), 
         data_coorte = stringr::str_sub(data_coorte, 1, 7))

# REAM 
ggplot(data = df_gasolina %>% 
         filter(!is.na(preco)) %>% 
         group_by(code_cidade) %>% 
         mutate(trat = if_else(sum(priv) >= 1, T, F)) %>% 
         ungroup() %>% 
         filter((trat == 0)|(trat == 1 & UF == "13")) %>% 
         summarise(preco_med = mean(preco_def), .by = c("trat","data")),
           aes(x = data, y = preco_med, group = factor(trat), color = factor(trat)))+
  geom_point()+
  geom_line()+
  labs(color = "Tratamento?")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))+
  geom_vline(data = coorte_gas %>% filter(data_coorte == "2022-12"), aes(xintercept = data_coorte), linetype = 2)
...
ggplot(data = df_gasolina %>% 
  ...
  labs(color = "Tratamento?")+
  theme(axis.text.x = element_text(angle = 90))+
  geom_vline(data = coorte_gas, aes(xintercept = data_coorte), linetype = 2)
```
...
df_gasolina %>% 
  ...
  scale_color_manual(values = distinct_colors)+
  labs(color = "Cidade",
       title = "Preços: PETROBRAS - Gasolina A")+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))
...
df_gasolina %>%
  mutate(Cidade = paste(Cidade, abbrev_state, sep = " - ")) %>% 
         filter(refinaria == "acelen") %>%
         group_by(Cidade) %>%
         reframe("Preço Médio" = round(mean(preco_def), 2)) %>% 
  kableExtra::kbl(centering = T, 
                  align = c("l", "c")) %>%
  kableExtra::kable_classic_2(full_width = F, html_font = "Calibri")
```
...
#          mutate(preco_med = mean(preco)) %>% 
#          rename("Preço Médio" = preco_med,
#                 Data = data) %>% mutate(Cidade = paste(Cidade, abbrev_state, sep = " - ")),
#        aes(x = Data, y = `Preço Médio`, group = factor(code_cidade), color = factor(Cidade)))+
#   geom_point()+
#   geom_line()+
#   scale_color_manual(values = distinct_colors)+
...
df_gasolina %>%
  mutate(Cidade = paste(Cidade, abbrev_state, sep = " - ")) %>% 
         filter(refinaria == "petrobras") %>%
         group_by(Cidade) %>%
         reframe("Preço Médio" = round(mean(preco_def, na.rm = T), 2)) %>% 
  kableExtra::kbl(centering = T, 
                  align = c("l", "c")) %>%
  kableExtra::kable_classic_2(full_width = F, html_font = "Calibri")
...
sumario_preco <- sumario_preco[c(9:12, 1:8),]
```
...
sumario_preco %>% 
  ...
  select(abbrev_state, Produto, date, everything()) %>% 
  ...
  kableExtra::kbl(centering = T, 
                  align = rep("c", 7)) %>%
  kableExtra::kable_classic_2(full_width = F, html_font = "Calibri") %>%
  kableExtra::row_spec(seq(4, nrow(sumario_preco), 4), extra_css = "border-bottom: 1px solid;") 
```
